...
function renderBoard(board) {
    allRow.forEach((row, i)=>{
        ...
        allCells.forEach((cell, j)=>{
            ...
            if (valeCell > 0) cell.classList.add(`field-cell--${valeCell}`);
        });
    });
    udateRec();
}
...
function statusCheker() {
    cleanAllMes();
    if (game.getStatus() === 'win') addMesage(winMasseg);
    if (game.getStatus() === 'lose') addMesage(loseMasseg);
}
// обробка початку гри
...
class Game {
    ...
    moveLeft() {
        ...
        this.board.forEach((row)=>{
            ...
            if (copyRow.toLocaleString() !== readyRow.row.toLocaleString()) chengCheck = true;
        });
        if (chengCheck) {
            this.board = rowMain;
            this.addRandomTitle();
    ...
    winChek(nowBoard) {
        nowBoard.forEach((row)=>{
            if (row.some((num)=>num === 2048)) this.status = 'win';
        });
    }
    ...
    loseChek(curentBoard) {
        ...
        if (!cleanCell) for(let i = 0; i < curentBoard.length; i++){
            for(let j = 0; j < curentBoard[i].length - 1; j++)if (curentBoard[i][j] === curentBoard[i][j + 1]) loseChek = false;
        }
        if (!cleanCell) {
            const columsBoard = this.creatColumn(curentBoard);
            for(let i = 0; i < columsBoard.length; i++){
                for(let j = 0; j < columsBoard[i].length - 1; j++)if (columsBoard[i][j] === columsBoard[i][j + 1]) loseChek = false;
            }
        }
        if (loseChek) this.status = 'lose';
    }
    // функция для смещения чисел
    move(row) {
        const newRow = [];
        row.forEach((num)=>{
            if (num !== 0) newRow.push(num);
        });
    ...
    creatColumn(board) {
        ...
        for(let i = 0; i < board.length; i++)for(let j = 0; j < board[i].length; j++)newColums[j].push(board[i][j]);
        return newColums;
    }
    ...
    parsRowInColums(colums) {
        ...
        for(let i = 0; i < colums.length; i++)for(let j = 0; j < colums[i].length; j++)readyBoard[j].push(colums[i][j]);
        return readyBoard;
    }
    ...
    slideRowRight(row) {
        const readyRow = this.slideRowLeft(row);
        return {
            row: readyRow.row.reverse(),
            record: readyRow.record
        };
    }
    // движения в верх
    slideRowUp(colums) {
        ...
        colums.forEach((column)=>{
            ...
            if (copyColumn.toString() !== newColumn.row.toString()) chengCheck = true;
        });
